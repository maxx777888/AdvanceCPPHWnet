...
public:
    ...
    std::unique_ptr<tridiagonal_matrix> clone() {
        //Один способ сделать копию объекта класса
        //auto smart_ptr_clone = std::unique_ptr<tridiagonal_matrix>(new tridiagonal_matrix(*this));
        //Второй способ сделать копию через метод std::make_unique 
        auto smart_ptr_clone = std::make_unique<tridiagonal_matrix>(*this);
